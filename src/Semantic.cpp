...
Expr* SemanticAnalyzer::makeDefaultExprForType(const std::shared_ptr<Type>& type, int line, int column) {
    if (!type) return nullptr;  //  Ставим всем инициализаторам default значение по умолчанию
    ...
    switch (type->kind) {
        case TypeKind::Bool: {
            auto* node = new Bool();
            node->value = false;    //  У bool это false
            expr = node;
            break;
        }
        ...
        case TypeKind::String: {
            ...
            node->value = "NULL";   // Ну и так далее
            expr = node;
            break;
        }
...
static ParamInfo makeParamInfo(const Param& param, const std::shared_ptr<Type>& paramType) {
    ParamInfo info;     //  Прослойка заполняющая данные о параметрах функции
...
FieldInfo* findFieldInTypeSymbol(std::shared_ptr<Symbol> sym, const std::string& fieldName) {
    if (!sym) {     //  При обращении к полю структуры, класса или namespace ищем данное поле у символа сущности
        return nullptr;
    }
...
std::shared_ptr<Type> SemanticAnalyzer::resolveTypeName(TypeName* typeName) {
    ...
    std::shared_ptr<Type> baseType;     //  Здесь выводим тип из имени 
...
void SemanticAnalyzer::analyzeStmt(Stmt* stmt) {
    ...
    if (auto* var = dynamic_cast<VarDecl*>(stmt)) { //  Если это объявление переменной
        std::shared_ptr<Type> declaredType;     //  Тип общий на весь список переменных (для не-auto)
...
std::expected<void, std::string> SemanticAnalyzer::analyzeModule(Program* program, const std::string& filePath, bool requireMain) {
    ...
    for (auto* decl : program->decls) { //  Четвёртый проход, осмотр самих тел функций
        if (dynamic_cast<ImportDecl*>(decl)) {  //  На этом этапе уже пропускаем импорты
            continue;
        }
